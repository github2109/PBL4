...
public class CiscoClone extends JFrame implements MouseMotionListener, MouseListener {
	...
	static Vector<Router> Routers = new Vector<Router>();
	...
	public static Cable getCableOnScreen(int X, int Y) {
		for (int i = 0; i < Cables.size(); i++) {
			Router[] routers = Cables.get(i).getRouters();
				...
				if((X - xR0 == 0 && xR1 - X == 0) || (Y - yR0 == 0 && yR1 - Y == 0)) return Cables.get(i);
				double k1 = (1.0 * (X - xR0) / (xR1 - X));
				double k2 = (1.0 * (Y - yR0) / (yR1 - Y));
				if (k1 / k2 < 1.2 && k1 / k2 > 0.8)
					return Cables.get(i);
		}
		return null;
	}

	public static void removeRouter(Router router) {
		for (int i = 0; i < Cables.size(); i++) {
			if (Cables.get(i).isConnectedRouters(router)) {
				Cables.remove(i);
				i--;
			}
		}
	...
	public void mouseReleased(MouseEvent e) {
		if (this.device == Device.ROUTER) {
			...
			router.getRouterLabel().addMouseListener(new MouseListener() {
				...
				public void mouseReleased(MouseEvent e) {
					...
					if (device == Device.CABLE_START) {
						...
						tmpCable = null;
						repaint();
					} else if (device == Device.REMOVE) {
						contentPane.remove(router.getRouterLabel());
						removeRouter(router);
						repaint();
					}
				}
			...
			if (getCableOnScreen(e.getX(), e.getY()) != null) {
				Cable cable = getCableOnScreen(e.getX(), e.getY());
