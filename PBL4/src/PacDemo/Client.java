...
public class Client extends JFrame implements Runnable {
	...
	public Client(int[][] Matrix) {
		...
		txtMap.setTabSize(10);
		txtMap.setFont(new Font("Tahoma", Font.PLAIN, 15));
		txtMap.setBackground(Color.WHITE);
	...
	public void run() {
		while (true) {
			try {
				...
				for(int i = 0 ;i<data.getPred().length;i++) {
					if(i == source) continue;
					map += i+1 + " : ";
					for(int j = 0 ;j<data.getPred()[i].size();j++) {
						map += data.getPred()[i].get(j) + 1 + " ";
						if(j != data.getPred()[i].size() - 1) map += "-> ";
					}
					map+="\n";
				}
				txtMap.setText(map);
